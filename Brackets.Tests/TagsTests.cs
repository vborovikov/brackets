...
public class TagsTests
{
    ...
    [TestMethod]
    public void Enumerate_RemoveAndAddOnEachIteration_ChildrenEnumerated()
    {
        var document = Document.Html.Parse("<div><span>1</span><span>2</span><span>3</span></div>");
        var newTag = (ParentTag)Document.Html.CreateTag("div");
        var parent = document.First() as ParentTag;
        foreach (var child in parent.Enumerate())
        {
            parent.Remove(child);
            newTag.Add(child);
        }
    ...
    [TestMethod]
    public void EnumerateOfT_RemoveAndAddOnEachIteration_ElementsEnumerated()
    {
        var document = Document.Html.Parse("<div><span>1</span><span>2</span><span>3</span></div>");
        var newTag = (ParentTag)Document.Html.CreateTag("div");
        var parent = document.First() as ParentTag;
        foreach (var span in parent.Enumerate<ParentTag>())
        {
            parent.Remove(span);
            newTag.Add(span);
        }
